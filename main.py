...
logger.setLevel(logging.DEBUG)  # Set the desired logging level to DEBUG
...
logging.getLogger('scipy').setLevel(logging.WARNING)  # Suppress DEBUG logs from scipy
...
if not LOGGING:
    logging.disable(logging.CRITICAL)  # Disable logging to the console if LOGGING is False
...
def get_cluster_id(x: int, y: int, node_num: int) -> int:
    ...
    :param x: X-coordinate of the node
    :param y: Y-coordinate of the node
    :param node_num: node number for logging
